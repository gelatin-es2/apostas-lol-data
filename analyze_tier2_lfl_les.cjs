// Tier 2 LFL + LES Split 2 analysis
// Output: cron-data/tier2_lfl_les_stats.json
// Filtro: Split 2 2026 (datas >= 2026-04-01)
// Resultado: missed opportunities pra inserir como SIMULATED no Banco de dados
...
const SPLIT1_START = '2026-04-01'; const SPLIT1_END = '2026-12-31';
const PEEL = ['Soraka','Sona','Janna','Lulu','Yuumi','Karma','Seraphine','Renata','RenataGlasc','Nami','Milio'];
const STAKE = 1000;
const ODD = 1.72;
const FALLBACK_LINE = 27.5;
const MIN_SAMPLE_TEAM = 4;
const FAIR_ADJUSTMENT = 0;
  ...
  for (const lg of LEAGUES) {
    ...
    for (let pi = 0; pi < 5; pi++) {
      ...
      for (const e of r.data.schedule.events) {
        ...
        if (e.state === 'completed' && date >= SPLIT1_START && date <= SPLIT1_END) {
      ...
      if (oldestDate < SPLIT1_START) break; // já passou do recorte
...
async function fetchUserBets() {
  ...
  return new Promise((resolve) => {
    // Split 1: CEO não tinha bets nesse período (off no Split 1). Bets query restrita à janela [SPLIT1_START, SPLIT1_END]
    const u = new URL(`${supabaseUrl}/rest/v1/bets?select=id,pick,odd,status,map_number,raw_extraction&bet_datetime=gte.${SPLIT1_START}&bet_datetime=lte.${SPLIT1_END}&status=in.(green,red)&bookmaker=neq.SIMULATED&limit=2000`);
...
(async () => {
  ...
  console.error(`  ${matches.length} matches completed >= ${SPLIT1_START}`);
  ...
  const outFile = path.join(outDir, 'tier2_lfl_les_stats.json');
  ...
  const ml = {
    generated_at: new Date().toISOString(),
    split_label: `${SPLIT1_START} → ${games.length ? games.map(g => g.date).sort().pop() : '?'}`,
    leagues: LEAGUES.map(l => l.name),
    cutoff_date: SPLIT1_START,
    min_games: MIN,
