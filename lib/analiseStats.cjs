...
function aggBy(simulated, keyFn, stake, odd) {
  ...
  for (const { b, sim } of simulated) {
    const gid = b.raw_extraction?.match_context?.lolesports_game_id
      ? String(b.raw_extraction.match_context.lolesports_game_id)
      : `NOID-${b.id || b.bet_id || Math.random()}`;
    const keys = keyFn(b);
    // Usa odd real da bet pra bets reais; fallback pro param `odd` apenas em SIMULATED sem odd
    const realOdd = parseFloat(b.odd);
    const usedOdd = (b.bookmaker !== 'SIMULATED' && !isNaN(realOdd) && realOdd > 1) ? realOdd : odd;
    for (const k of keys) {
      if (!k) continue;
      const mapKey = k + '|' + gid;
      if (!byMap.has(mapKey)) byMap.set(mapKey, { name: k, profit: 0 });
      const entry = byMap.get(mapKey);
      entry.profit += sim.won ? stake * (usedOdd - 1) : -stake;
    }
  }
