...
// Fix 2026-07-30 (auditoria under/over): extrai a linha de kills do texto do pick.
// Bug antigo pegava o PRIMEIRO número da string ("Mapa 2 Menos de 26.5" → 2, o número
// do mapa, não a linha). Prioriza o padrão decimal N.5 (linha de kills sempre termina
// em .5); só cai pro fallback "número após o termo de mercado" se não achar decimal.
...
// Fix 2026-07-30 (auditoria under/over): decide over/under pelo TERMO do pick, nunca
// por substring do nome do mercado. Bug antigo casava "over" dentro de picks tipo
// "Under 28.5 Total Kills Over/Under, Map 4" (nome de mercado completo salvo junto)
// e virava isOver=true numa aposta que era Under. under/menos sempre tem precedência;
// "Mapa N " prefixado (BO3/BO5) é ignorado antes de olhar o termo.
function pickIsOver(pickRaw) {
  const s = (pickRaw || '').replace(/^mapa\s*\d+\s*/i, '').trim();
  if (/^(under|menos de)\b/i.test(s)) return false;
  if (/^(over|mais de)\b/i.test(s)) return true;
  // fallback (pick não começa com o termo): under/menos ainda vence se aparecer
  if (/\b(under|menos de)\b/i.test(s)) return false;
  return /\b(over|mais de)\b/i.test(s);
}
...
async function fetchAndDedup(supabaseUrl, supabaseKey) {
  ...
  // Fix 2026-07-30: tie-break determinístico. bet_datetime sozinho tem 112+ grupos
  // com valor duplicado (ladders/duplas) — sem 2º critério, a ordem de empate é
  // indefinida no Postgres e pode variar entre execuções, fazendo "1ª bet do mapa"
  // (que decide o resultado em aggBy) flipar. created_at.desc + id.desc fecham o empate.
  const endpoint =
    '/rest/v1/bets' +
    '?select=*' +
    '&status=in.(green,red)' +
    '&is_method_bet=eq.true' +
    '&order=bet_datetime.desc,created_at.desc,id.desc' +
    '&limit=2000';
...
function getBetSimulation(b, delta, odd) {
  ...
  const isOver = pickIsOver(b.pick);
...
async function fetchAnaliseStats(supabaseUrl, supabaseKey, opts = {}) {
  ...
  const meta = {
    ...
    query: `/rest/v1/bets?select=*&status=in.(green,red)&is_method_bet=eq.true&order=bet_datetime.desc,created_at.desc,id.desc&limit=2000`,
  };

  return { teams, leagues, meta };
}

module.exports = { fetchAnaliseStats, getBetSimulation, aggBy, parsePickLine, pickIsOver };
