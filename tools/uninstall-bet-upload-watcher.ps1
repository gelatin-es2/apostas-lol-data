# uninstall-bet-upload-watcher.ps1
# DESABILITA a tarefa do Bet Upload Watcher (ApostasLoL-BetUploadWatcher) sem remove-la:
# a definicao/config/historico ficam intactos no Agendador, so param de disparar. Rodar
# tools\install-bet-upload-watcher.ps1 de novo re-habilita com a config mais recente.
#
...
#
# Uso:  powershell -ExecutionPolicy Bypass -File tools\uninstall-bet-upload-watcher.ps1
...
Start-Sleep -Seconds 1
$orfaos = @(Get-CimInstance Win32_Process -Filter "Name='node.exe'" -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -and $_.CommandLine -like "*$WatcherPath*" })

if ($orfaos.Count -gt 0) {
    Write-Host ""
    Write-Host "encontrei $($orfaos.Count) processo(s) node.exe do Bet Upload Watcher que ficaram orfaos - encerrando..." -ForegroundColor Yellow
    foreach ($p in $orfaos) {
...
Write-Host ""
Write-Host "Pronto: o modo automatico do Bet Upload Watcher foi desligado." -ForegroundColor Cyan
Write-Host "A tarefa continua registrada (desabilitada) - rode tools\install-bet-upload-watcher.ps1 pra religar." -ForegroundColor Cyan
