# install-bet-upload-watcher.ps1
# Cria (ou atualiza) e HABILITA a tarefa do Agendador de Tarefas que roda o Bet Upload
# Watcher sozinho, sem janela, uma vez por minuto (o watcher em si roda em modo --once
# por chamada e sai — nao e um processo 24/7 como o Vigia Pinnacle).
#
# Tarefa: ApostasLoL-BetUploadWatcher
#   - dispara a cada 1 min, comecando na instalacao, indefinidamente
#   - roda scripts\run-bet-upload-watcher-hidden.vbs via wscript, sem janela
...
# tinha sido desligada antes por tools\uninstall-bet-upload-watcher.ps1.
#
# Uso:  powershell -ExecutionPolicy Bypass -File tools\install-bet-upload-watcher.ps1
#
...
$TaskName    = 'ApostasLoL-BetUploadWatcher'
$ProjectRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$VbsPath     = Join-Path $ProjectRoot 'scripts\run-bet-upload-watcher-hidden.vbs'
$CmdPath     = Join-Path $ProjectRoot 'scripts\run-bet-upload-watcher.cmd'
$WatcherPath = Join-Path $ProjectRoot 'scripts\bet-upload-watcher.cjs'

Write-Host "== instalando autostart do Bet Upload Watcher ==" -ForegroundColor Cyan
...
try {
    Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $Trigger `
        ...
        -Description 'Roda o Bet Upload Watcher (registro de aposta por print via Claude Code headless) a cada 1 min, sem janela.' | Out-Null
} catch {
...
# Garante habilitada mesmo se a tarefa ja existia desligada por uninstall-bet-upload-watcher.ps1.
...
Write-Host ""
Write-Host "PRONTO. A partir de agora o Bet Upload Watcher roda 1x por minuto, sem janela," -ForegroundColor Cyan
Write-Host "processando no maximo um job de upload por chamada." -ForegroundColor Cyan
Write-Host "Log do worker (Claude Code): cron-data\bet-upload-work\logs\worker-AAAA-MM-DD.log" -ForegroundColor Cyan
Write-Host "Para desligar o automatico:  powershell -File tools\uninstall-bet-upload-watcher.ps1" -ForegroundColor Cyan
