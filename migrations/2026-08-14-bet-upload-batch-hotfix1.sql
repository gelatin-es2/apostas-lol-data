-- HOTFIX 1 (2026-08-14): register_bet_upload_batch
-- 1) allowlist de casas: + thunderpick, whale -> whale.io (rotulo canonico)
-- 2) ticket opcional tambem para thunderpick (painel Open nao mostra bilhete)
-- Reaplica a funcao inteira em transacao unica; nao toca em tabelas nem dados.
  ...
  loop
    begin
      ...
      if nullif(btrim(bet_doc ->> 'bookmaker'), '') is null
         or lower(btrim(bet_doc ->> 'bookmaker')) not in ('estrelabet', 'pinnacle', 'parimatch', 'betano', 'whale.io', 'thunderpick', 'polymarket')
      ...
      if lower(btrim(bet_doc ->> 'bookmaker')) not in ('polymarket', 'thunderpick')
...
select 'hotfix1 aplicado: thunderpick + whale.io no batch';
